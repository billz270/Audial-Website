...
**Next step:** founder pastes `RESEND_API_KEY` into `.env` → run `netlify dev` and place a real end-to-end test order (TEST folder + real email) with curl → then DEV-51 (the form + panel capture calling the three endpoints).
