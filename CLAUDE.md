...
- **Checkout flow** ✓ — Two-step checkout modal on both Configurator and Room Visualizer pages. "Checkout →" button in visualizer sidebar replaces "Clear All Panels" (moved to canvas area below the size hint, styled as `.clear-canvas-btn`). "Checkout →" button in configurator replaces "Request Quote →". Step 1 shows cart review (panels, thumbnails, prices, subtotal from `acousticCart`). Step 2 collects full order details (name, email, phone, address, pincode, city, notes). Submits to `rohan270@gmail.com` via Formspree (`xbdbzvao`). Success state shows the customer's email in the confirmation message. (`configurator.html`, `room-visualizer.html`)
